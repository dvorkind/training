...
public class TrackListUtils {
    ...
    public static TrackList sortSongsBy(TrackList tracklist, SortBy sortby) {
        ...
        case GENRE:
            ...
            break;
        case NAME:
            sortedTracklist.getTracks().sort(Comparator.comparing(Track::getSinger).thenComparing(Track::getTitle));
            break;
        case DURATION:
            sortedTracklist.getTracks().sort(Comparator.comparing(Track::getDuration).thenComparing(Track::getSinger));
            break;
        }
    ...
    public static TrackList findTrackByRange(int from, int to) {
        TrackList foundTracklist = new TrackList();
        for (int i = 0; i < DiskList.getInstance().getCurrentDisk().getTracklist().getTracks().size(); i++) {
            Track track = DiskList.getInstance().getCurrentDisk().getTracklist().getTracks().get(i);
            if (track.getDuration() >= from && track.getDuration() <= to) {
                foundTracklist.getTracks().add(track);
            }
        }
